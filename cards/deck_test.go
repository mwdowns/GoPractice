...
func TestNewDeck(t *testing.T) {
	...
	if d[0].name != "Ace of Hearts" {
		t.Errorf("Expected Ace of Hearts as first card, got %v", d[0].name)
	}
	fmt.Println("First card test passed")
	if d[len(d)-1].name != "Ten of Clubs" {
		t.Errorf("Expected Ten of Clubs as last card, got %v", d[len(d)-1].name)
	}
